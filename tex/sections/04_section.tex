...
Two qualifications keep the strength of this test in view. First, the $R^2 = 0.9999$ reconstruction is a near-definitional check rather than the discovery of an empirical regularity: the EPA standard \emph{defines} total emissions as the weighted sum of metered fuels, so a clean reconstruction confirms that the recorded data implement the documented identity and that no parent of $Y$ has been omitted, but it could not have surfaced a hidden cause the accounting itself does not name. Its evidential force lies in ruling out departures, gross data error, an unmetered fuel, a mis-stated factor, not in establishing a law; here the data conform, and that conformance is what licenses treating $\mathrm{pa}(Y) = M$ as established. Second, and more consequentially, the documentary test reaches the \emph{structure} of selection (which variables the gate keys on) but not its \emph{ignorability} (whether some unobserved driver of compliance also moves the outcome). The ordinance fixes $\mathbf{X}_s \to C$; it is silent on whether, conditional on $\mathbf{X}_s$, $X_o$, and the period index, compliance is independent of $Y$. That condition is what the analysis of Sections 5 to 7 rests on beyond the tested graph, and Section 8 carries it as the principal limitation. The phrase \emph{tested DGP} should be read accordingly: the graph's topology is tested against documents and corroborated by the data, while the no-unobserved-selection condition that licenses the back-door correction is assumed, not tested.

With those qualifications stated, all three predictions survive contact with documents that took no part in drawing the graph, and the two regressions add quantitative corroboration where the data can speak. The graph that results is a tested object rather than an asserted one (Figure 4.8): its selection edge is the ordinance's eligibility rule, its mediator/outcome split is the reporting schema's input/output distinction, and its $\mathrm{pa}(Y) = M$ claim is the EPA accounting identity, recovered from the data to $R^2 = 0.9999$. That is the warrant Sections 5 to 7 draw on when they run the full Pearlian analysis on the tested structure.
